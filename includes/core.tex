...
Vo svojej grafickej reprezentácií Petriho sieťe modelujú udalostné systémy pomocou bipartitného grafu ktorý sa skladá z miest(kruh) a prechodov (štvorec) ktoré sú pospájané orientovanými hranami(šipkami) s označením multiplicity. Rôzne stavy procesu značíme značkami alebo inak tokenmi (bodkami v miestach). 
Miesta (S) reprezentujú podmienky ktoré musia byť splnené na to aby mohla prebehnúť nejaká akcia v procese.
Prechody (T) reprezentujú samotné akcie v procese, ktoré menia jej stav konzumovaním a produkovaním tokenov.
Hrany (A) reprezentujú tok prostriedkov v procese a v spojení s miestami a prechodmi tvoria štruktúru procesu
Každá hrana má definovanú násobnosť (W) ktorá reprezentuje počet tokenov, ktoré sa pri spustení prechodu skonzumujú, ak hrana smeruje do prechodu, alebo vyprodukujú ak hrana vychádza z vrcholu. Na diagrame \ref{basic net} vidíme príklad Petriho siete ktorá modeluje jednoduchý paralelný proces. 
